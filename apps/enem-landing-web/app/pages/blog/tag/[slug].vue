...
import type { BlogTag, PaginatedBlogPosts } from '@enem-landing/shared-types';
...
const [{ data: paginated }, { data: tags }] = await Promise.all([
  useFetch<PaginatedBlogPosts>('/api/blog-posts', {
    query: { tag: slug, page },
    watch: [page],
  }),
  useFetch<BlogTag[]>('/api/blog-tags'),
]);

const tag = computed(() => tags.value?.find((item) => item.slug === slug));
if (!tag.value) {
...
useSeoMeta({
  title: computed(() => `#${tag.value?.name} - Blog`),
  description: computed(
    () => `Posts tagged #${tag.value?.name} on Nurfirliana Muzanella's blog.`,
  ),
});
</script>
