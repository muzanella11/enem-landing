...
# Membuat 4 database (2 app x 2 environment) di satu instance Aiven yang
# sama: enem_landing_account_dev, enem_landing_account_prod
# (enem-landing-account-api), enem_landing_api_dev, enem_landing_api_prod
# (enem-landing-api).
#
...
(async () => {
  ...
  const envs = ["dev", "prod"];
  const databases = envs.flatMap((env) => [
    `enem_landing_account_${env}`,
    `enem_landing_api_${env}`,
  ]);
