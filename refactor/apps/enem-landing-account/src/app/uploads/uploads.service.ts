...
export class UploadsService {
  ...
  private async getR2Config(): Promise<R2Config> {
    const [accessKeyId, secretAccessKey, endpoint, bucket, publicUrlBase] = await Promise.all([
  ...
  private buildClient(config: Pick<R2Config, 'accessKeyId' | 'secretAccessKey' | 'endpoint'>): S3Client {
    return new S3Client({
      region: 'auto',
  ...
  async upload(file: Express.Multer.File | undefined, dto: UploadFileDto, uploaderId: string) {
    return benchmark(`${this.constructorName}@upload`, async () => {
      if (!file) {
        throw new BadRequestException('File is required');
      }
      if (file.size > dto.maxSize) {
        throw new BadRequestException(`File size exceeds the maximum of ${dto.maxSize} bytes`);
      }
  ...
  async remove(id: string, requesterId: string) {
    return benchmark(`${this.constructorName}@remove`, async () => {
      ...
      const config = await this.getR2Config();
      const client = this.buildClient(config);
      await client.send(new DeleteObjectCommand({ Bucket: config.bucket, Key: file.key }));
