...
describe('LocalStrategy', () => {
  it('delegates credential validation to AuthService.validateUser', async () => {
    ...
    await expect(strategy.validate('admin@example.com', 'secret')).resolves.toBe(user);
    expect(authService.validateUser).toHaveBeenCalledWith('admin@example.com', 'secret');
  });
});
