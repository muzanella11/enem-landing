...
describe('AuthService', () => {
  let usersService: { findByEmail: ReturnType<typeof vi.fn>; findById: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn>; updatePassword: ReturnType<typeof vi.fn> };
  let jwtService: { sign: ReturnType<typeof vi.fn> };
  let redisService: { setValue: ReturnType<typeof vi.fn>; deleteValue: ReturnType<typeof vi.fn>; getValue: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    redisService = { setValue: vi.fn(), deleteValue: vi.fn(), getValue: vi.fn() };

    service = new AuthService(usersService as never, jwtService as never, redisService as never);
  ...
  describe('validateUser', () => {
    it('returns the user when the password matches', async () => {
      ...
      await expect(service.validateUser('admin@example.com', 'secret')).resolves.toBe(user);
    });

    it('throws when the user does not exist', async () => {
      usersService.findByEmail.mockResolvedValue(null);
      await expect(service.validateUser('missing@example.com', 'secret')).rejects.toThrow(
    ...
    it('throws when the password does not match', async () => {
      ...
      await expect(service.validateUser('admin@example.com', 'wrong')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  });
  ...
  describe('whoami', () => {
    it('returns the JWT payload as-is', async () => {
      const payload = { id: 'user-1', fullname: 'Test Admin', email: 'admin@example.com', role: Role.Admin };
  ...
  describe('signout', () => {
    it('deletes the Redis session key', async () => {
      ...
      expect(redisService.deleteValue).toHaveBeenCalledWith('auth:user-1:token');
    });
  });
  ...
  describe('signup', () => {
    it('is disabled by default (ALLOW_SIGNUP unset)', async () => {
      await expect(
        service.signup({ fullname: 'New User', email: 'new@example.com', password: 'password123' }),
      ).rejects.toThrow(ForbiddenException);
    });
  });
});
