...
describe('UsersService', () => {
  ...
  it('findByEmail queries by email', async () => {
    ...
    expect(repo.findOne).toHaveBeenCalledWith({ where: { email: 'a@example.com' } });
  });

  it('create persists a new user via the repository', async () => {
    const data = { fullname: 'A', email: 'a@example.com', passwordHash: 'hash', role: Role.Admin };
  ...
  it('upsertByEmail creates when no existing user is found', async () => {
    ...
    const data = { fullname: 'A', email: 'a@example.com', passwordHash: 'hash', role: Role.User };
  ...
  it('upsertByEmail merges into the existing user when found', async () => {
    ...
    const data = { fullname: 'New Name', email: 'a@example.com', passwordHash: 'new-hash', role: Role.User };
